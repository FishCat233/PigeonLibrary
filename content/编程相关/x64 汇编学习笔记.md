...
mov rax, 5 ;# 立即数赋值寄存器.
mov rbx, rax ;# 用寄存器赋值
;# mov rip, 00401000 ;# 不能 mov rip 寄存器, 因为 rip 实际上是存放下一条指令地址的寄存器
```
...
调用操作系统内核的 call，在 Linux 上，`rax` 存放 call 函数码，然后参数从前往后分别是 `rdi` / `rsi` / `rdx` / `rcx` / `r8` / `r9`，随后多余的参数压栈。
...
而关于函数调用的传参和返回值，通常会有几套规范来统一大家写的代码如何互相调用，这里使用的时 System V ABI，也就是之前所说的 `rdi` / `rsi` / ... 那套，同时函数返回值存在 `rax` 里。

...
**约束条件**
$X$ 需要满足以下约束：
- 次数约束：$x_{ij}$ 仅为 $0$ \text{或} $1$ \text{并且} $\forall j, \sum_{i=0}^2x_{ij}\leq 1$
- 时间约束：$\forall i, \sum_{j}^{8}x_{ij} * A_{j 0}\leq 10$
- 费用约束：$\sum_{i,j}^{2,8} x_{ij} * A_{j 1} \leq 800$
- 地理约束：
	- $\text{当}x_{i_{6}}=1\text{时，}\sum_{j=0}^{8}x_{ij}=1$.
	- $\text{当}x_{i 1}=1\text{时} \sum_{2}^5x_{ij}\geq 1 \text{并且} x_{i 0}+x_{i 7}=0$
